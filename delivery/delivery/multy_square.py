...
class OffboardControl(Node):
    ...
    def check_all_drones_arrived(self):
        distance_to_target = ((self.target_position[0] - self.local_position_sub.position[0]) ** 2 +
                              (self.target_position[1] - self.local_position_sub.position[1]) ** 2 +
                              (self.takeoff_altitude - self.current_altitude) ** 2) ** 0.5

        if distance_to_target < 0.5 and not self.arrived:
            self.arrived = True
            OffboardControl.completed_drones += 1
    # 새로운 노드를 동시에 생성해서 값을 발행하고 숫자가 4가 되면 이를 실시간 구독하여 다음으로 넘어가기
    # 또는 키보드 1번, 2번 등 teleop_key 적용
