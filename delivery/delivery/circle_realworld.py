...
class OffboardControl(Node):
    def __init__(self):
        ...
        self.stop_duration = 5.0  # seconds 한 바퀴 돌고 멈춤.
        ...
        if self.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            ...
            if not self.takeoff_complete:
                ...
                if self.current_altitude <= self.takeoff_altitude:
                    ...
                    self.stop_start_time = self.get_clock().now().seconds_nanoseconds()[0]  # Record
            if self.takeoff_complete:
                ...
                if self.stop_state:
                    # Calculate the desired position for continuous circumferential flight
                    radius = 5.0  # meters
                    angular_speed = 0.1  # rad/s
                    desired_position = np.array([self.current_position[0] + radius * np.cos(self.current_heading),
                                                 self.current_position[1] + radius * np.sin(self.current_heading),
                                                 self.current_altitude])

                    # Calculate the desired heading
                    desired_heading = self.current_heading + angular_speed * self.dt

                    # Calculate the desired velocity
                    desired_velocity = (desired_position - self.current_position) / self.dt
                    ...
                    self.current_speed = np.linalg.norm(desired_velocity)
